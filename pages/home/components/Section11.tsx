import Image from "next/image";

const Section11 = () => {
  return (
    <div className="section-large-padding bg-gradient-white-to-pink wf-section">
      <div className="wrap">
        <div className="text-center text-left-on-mobile mb-40">
          <h2>
            Rejoins les entrepreneurs <br />
            <span className="color-text-gradient">
              qui ont propulsé leur projet avec
            </span>
          </h2>
        </div>
        <div className="testimonial-key-number-container">
          <div
            id="w-node-f8977c7f-d4cf-f19e-bea1-fe4c9236a70c-9236a703"
            className="key-number-container"
          >
            <div className="xxxl extra-bold color-text-strong">+20M</div>
            <div className="s bold uppercase">euros levés par nos clients</div>
          </div>
          <div className="divider-vertical-small-pink hide-on-mobile" />
          <div
            id="w-node-f8977c7f-d4cf-f19e-bea1-fe4c9236a712-9236a703"
            className="key-number-container"
          >
            <div className="xxxl extra-bold color-text-strong">+120</div>
            <div className="s bold uppercase">startups propulsées</div>
          </div>
          <div className="divider-vertical-small-pink hide-on-mobile" />
          <div
            id="w-node-f8977c7f-d4cf-f19e-bea1-fe4c9236a718-9236a703"
            className="key-number-container"
          >
            <div className="xxxl extra-bold color-text-strong">+3</div>
            <div className="s bold uppercase">
              années d&apos;expérience avec les startups
            </div>
          </div>
        </div>
      </div>
      <div className="wrap">
        <div className="testimonials-grid">
          <div
            data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
            className="card shadow-pink-medium"
            style={{
              opacity: 1,
              transform:
                "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
              transformStyle: "preserve-3d",
            }}
          >
            <div className="testimonial-header">
              <div className="testimonial-picture-container">
                <Image
                  src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c47bd749cf6fa112f9c7d2_Christophe%20Bellagamba.png"
                  loading="lazy"
                  alt="Photo Christophe"
                  className="testimonial-picture"
                />
                <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                    loading="lazy"
                    alt=""
                  />
                </div>
              </div>
              <div>
                <div className="color-text-strong bold">
                  Christophe Bellagamba
                </div>
                <div className="xs">Founder</div>
              </div>
              <a
                href="https://www.linkedin.com/in/christophebellagamba/"
                className="w-inline-block"
              >
                <Image
                  src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                  loading="lazy"
                  alt="Logo LinedIn"
                />
              </a>
            </div>
            <p className="s">
              &#34;Les différentes interactions avec Mathieu et Rémi étaient
              très fluides et l&apos;atelier d&apos;1h30 consacré à l&apos;UX
              est particulièrement utile. Je suis très satisfait du rendu
              final.&#34;
            </p>
          </div>
          <div
            data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
            className="card shadow-pink-medium"
            style={{
              opacity: 1,
              transform:
                "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
              transformStyle: "preserve-3d",
            }}
          >
            <div className="testimonial-header">
              <div className="testimonial-picture-container">
                <Image
                  src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c47bd7edbbc3c1b26054ae_Etienne.png"
                  loading="lazy"
                  alt="Photo Etienne"
                  className="testimonial-picture"
                />
                <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                    loading="lazy"
                    alt=""
                  />
                </div>
              </div>
              <div>
                <div className="color-text-strong bold">
                  Etienne de Sainte Marie
                </div>
                <div className="xs">CEO @Republike</div>
              </div>
              <a
                href="https://www.linkedin.com/in/edesaintemarie/"
                className="w-inline-block"
              >
                <Image
                  src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                  loading="lazy"
                  alt="Logo LinedIn"
                />
              </a>
            </div>
            <p className="s">
              &#34;La formule peut permettre de dégrossir une énorme quantité de
              points en suspens si ce n&apos;est pas tout simplement
              d&apos;avoir une version prête pour un go-to-market.&#34;
            </p>
          </div>
          <div
            data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
            className="card shadow-pink-medium"
            style={{
              opacity: 1,
              transform:
                "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
              transformStyle: "preserve-3d",
            }}
          >
            <div className="testimonial-header">
              <div className="testimonial-picture-container">
                <Image
                  src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c47bd5be2eeccc55460a05_Alexy.png"
                  loading="lazy"
                  alt="Photo Alexy"
                  className="testimonial-picture"
                />
                <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                    loading="lazy"
                    alt=""
                  />
                </div>
              </div>
              <div>
                <div className="color-text-strong bold">Alexy Creusot</div>
                <div className="xs">CEO @VosTalents</div>
              </div>
              <a
                href="https://www.linkedin.com/in/alexy-creusot/"
                className="w-inline-block"
              >
                <Image
                  src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                  loading="lazy"
                  alt="Logo LinedIn"
                />
              </a>
            </div>
            <p className="s">
              &#34;Une équipe dynamique, une méthodologie rodée et un résultat à
              100% en phase avec nos attentes. Excellent et encore merci pour ce
              coût de boost !&#34;
            </p>
          </div>
          <div
            data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
            className="card shadow-pink-medium"
            style={{
              opacity: 1,
              transform:
                "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
              transformStyle: "preserve-3d",
            }}
          >
            <div className="testimonial-header">
              <div className="testimonial-picture-container">
                <Image
                  src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c47bd7be2eec0473460a06_Mathis.png"
                  loading="lazy"
                  alt="Photo Mathis"
                  className="testimonial-picture"
                />
                <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                    loading="lazy"
                    alt=""
                  />
                </div>
              </div>
              <div>
                <div className="color-text-strong bold">Mathis Bois</div>
                <div className="xs">CTO @StridePower</div>
              </div>
              <a
                href="https://www.linkedin.com/in/mathisbois/"
                className="w-inline-block"
              >
                <Image
                  src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                  loading="lazy"
                  alt="Logo LinedIn"
                />
              </a>
            </div>
            <p className="s">
              &#34;L&apos;équipe a compris très vite notre vision et a eu
              l&apos;inspiration que l&apos;on espérait. L&apos;état
              d&apos;esprit est top ! Le travail livré est soigné et
              rapide.&#34;
            </p>
          </div>
          <div className="hide-on-mobile">
            <div
              data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
              className="card shadow-pink-medium"
              style={{
                opacity: 1,
                transform:
                  "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
                transformStyle: "preserve-3d",
              }}
            >
              <div className="testimonial-header">
                <div className="testimonial-picture-container">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c47bd7fcd85222d17e223d_Edwige.png"
                    loading="lazy"
                    alt="Photo Edwige"
                    className="testimonial-picture"
                  />
                  <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                    <Image
                      src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                      loading="lazy"
                      alt=""
                    />
                  </div>
                </div>
                <div>
                  <div className="color-text-strong bold">Edwige Michau</div>
                  <div className="xs">CEO @Barooders</div>
                </div>
                <a
                  href="https://www.linkedin.com/in/edwige-michau-02a8735a/"
                  className="w-inline-block"
                >
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                    loading="lazy"
                    alt="Logo LinedIn"
                  />
                </a>
              </div>
              <p className="s">
                &#34;J&apos;ai eu l&apos;occasion de travailler avec à plusieurs
                reprises. Ils ont su m&apos;écouter et me conseiller. C&apos;est
                une équipe très professionnelle, avec une grande expertise en
                matière de design.&#34;
              </p>
            </div>
          </div>
          <div className="hide-on-mobile">
            <div
              data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
              className="card shadow-pink-medium"
              style={{
                opacity: 1,
                transform:
                  "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
                transformStyle: "preserve-3d",
              }}
            >
              <div className="testimonial-header">
                <div className="testimonial-picture-container">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c47c5ce3e4a8c3b092e1f6_Yann.png"
                    loading="lazy"
                    alt="Photo Yann"
                    className="testimonial-picture"
                  />
                  <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                    <Image
                      src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                      loading="lazy"
                      alt=""
                    />
                  </div>
                </div>
                <div>
                  <div className="color-text-strong bold">Yann Petretti</div>
                  <div className="xs">CEO @Blend</div>
                </div>
                <a
                  href="https://www.linkedin.com/in/yannpetretti/"
                  className="w-inline-block"
                >
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                    loading="lazy"
                    alt="Logo LinedIn"
                  />
                </a>
              </div>
              <p className="s">
                &#34;Travailler avec c&apos;est confier votre idée encore
                confuse à des génies créatifs qui comprennent parfaitement votre
                business et la traduisent en maquettes de la plus haute qualité.
                Je recommande !&#34;
              </p>
            </div>
          </div>
          <div className="hide-on-mobile">
            <div
              data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
              className="card shadow-pink-medium"
              style={{
                opacity: 1,
                transform:
                  "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
                transformStyle: "preserve-3d",
              }}
            >
              <div className="testimonial-header">
                <div className="testimonial-picture-container">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c47bd5fcd85257d97e223a_Christophe%20Bellagamba-3.png"
                    loading="lazy"
                    alt="Photo Stéphane"
                    className="testimonial-picture"
                  />
                  <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                    <Image
                      src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                      loading="lazy"
                      alt=""
                    />
                  </div>
                </div>
                <div>
                  <div className="color-text-strong bold">
                    Stéphane Bernardo
                  </div>
                  <div className="xs">CEO @AloaMemoryBox</div>
                </div>
                <a
                  href="https://www.linkedin.com/in/st%C3%A9phane-bernardo-new-oxatis-account-executive/"
                  className="w-inline-block"
                >
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                    loading="lazy"
                    alt="Logo LinedIn"
                  />
                </a>
              </div>
              <p className="s">
                &#34;, c’est une expérience client au top proposée par une
                équipe réactive qui rend un sujet technique très abordable.&#34;
              </p>
            </div>
          </div>
          <div className="hide-on-mobile">
            <div
              data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
              className="card shadow-pink-medium"
              style={{
                opacity: 1,
                transform:
                  "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
                transformStyle: "preserve-3d",
              }}
            >
              <div className="testimonial-header">
                <div className="testimonial-picture-container">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c47bd50aeeec1e9df1f782_Christophe%20Bellagamba-2.png"
                    loading="lazy"
                    alt="Photo Nicolas"
                    className="testimonial-picture"
                  />
                  <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                    <Image
                      src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                      loading="lazy"
                      alt=""
                    />
                  </div>
                </div>
                <div>
                  <div className="color-text-strong bold">Nicolas Romero</div>
                  <div className="xs">CEO @TheFuture</div>
                </div>
                <a
                  href="https://www.linkedin.com/in/santoromero/"
                  className="w-inline-block"
                >
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                    loading="lazy"
                    alt="Logo LinedIn"
                  />
                </a>
              </div>
              <p className="s">
                &#34;Très content de la prestation de . Rapide, efficace et
                organisé. C&apos;est le service idéal pour poser des bases
                solides.&#34;
              </p>
            </div>
          </div>
          <div className="hide-on-tablet">
            <div
              data-w-id="649e2b46-733a-239c-4138-8388afc34db6"
              className="card shadow-pink-medium"
              style={{
                opacity: 1,
                transform:
                  "translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
                transformStyle: "preserve-3d",
              }}
            >
              <div className="testimonial-header">
                <div className="testimonial-picture-container">
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/626c66b0c2f2792e3c2ced9f_unnamed.png"
                    loading="lazy"
                    alt=""
                    className="testimonial-picture"
                  />
                  <div className="bullet-testimonial bullet-tesrimonial-in-picture">
                    <Image
                      src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c22f53889de07f66f0086f_quote.svg"
                      loading="lazy"
                      alt=""
                    />
                  </div>
                </div>
                <div>
                  <div className="color-text-strong bold">Célia Ricca</div>
                  <div className="xs">Senior PM @EtOH</div>
                </div>
                <a
                  href="https://fr.linkedin.com/in/celia-ricca"
                  className="w-inline-block"
                >
                  <Image
                    src="https://assets.website-files.com/61129ba17ba6a39d24a9733d/61c3259250204405c887d0f6_icon-linkedin.svg"
                    loading="lazy"
                    alt="Logo LinedIn"
                  />
                </a>
              </div>
              <p className="s">
                &#34;Sur un projet très complexe en termes de brief et de
                compréhension, une organisation millimétrée pour une efficacité
                de compétition !&#34;
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Section11;
